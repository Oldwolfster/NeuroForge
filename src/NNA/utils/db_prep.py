...
def prep_RamDB():
    ...
    dummy_sample = RecordSample(run_id=0, epoch=0, sample_num=0, inputs="", target=0.1, prediction=0.1,
    ...
    db.add(dummy_neuron, exclude_keys={"activation", "output_neuron"}, run_id=0, epoch=0, sample_num=0)
    # db.execute("CREATE INDEX idx_model_epoch_sample_num ON Neuron (model, epoch, sample_num);")
    db.execute("CREATE INDEX idx_epoch_sample_num ON Neuron (run_id, epoch, sample_num);")
    db.execute("CREATE INDEX idx__RecordSample ON RecordSample (run_id,  sample_num);")
    ...
    db.execute(
        ...
        CREATE TABLE IF NOT EXISTS Weight (
            ...
            value REAL NOT NULL,            
            PRIMARY KEY (run_id, epoch, sample_num, nid, weight_id)       
        );""")
    ...
    db.execute("""
        CREATE TABLE IF NOT EXISTS ErrorSignalCalcs (
            ...
            PRIMARY KEY (run_id, epoch, sample_num,  nid,weight_id)  -- Ensures unique calculations per neuron per step
        );""")
