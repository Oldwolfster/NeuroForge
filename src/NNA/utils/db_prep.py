...
def prep_RamDB():
    ...
    """
    dummy_iteration = Iteration(run_id=0, epoch=0, iteration=0, inputs="", target=0.1, prediction=0.1,
                                inputs_unscaled="", target_unscaled=0.1, prediction_unscaled=0.1, prediction_raw=0.1,
                                loss=0.1, loss_gradient=0.1, loss_function="dummy", accuracy_threshold=0.0)
    dummy_neuron = Neuron(0, 1, 0.0, Initializer_Tiny, 0)
    db.add(dummy_iteration)

    db.add(dummy_neuron, exclude_keys={"activation", "output_neuron"}, run_id=0, epoch=0, iteration=0)
    # db.execute("CREATE INDEX idx_model_epoch_iteration ON Neuron (model, epoch, iteration);")
    db.execute("CREATE INDEX idx_epoch_iteration ON Neuron (run_id, epoch, iteration);")
